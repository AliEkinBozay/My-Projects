...
<head>
    ...
    <%
        ...
        Connection con = null;
        try {
            con = hastaDBconn.getConnection();
            Statement st = con.createStatement();
            ResultSet rs = st.executeQuery("SELECT dp.departmentName, d.doctorName, d.doctorSurname FROM doctors d JOIN departments dp ON d.departmentID = dp.departmentID");

            while (rs.next()) {
                temp = new String[2]; // Initialize temp array here
                temp[0] = rs.getString("dp.departmentName");
                String name = rs.getString("d.doctorName") + " " + rs.getString("d.doctorSurname");
                temp[1] = name;
                doctors.add(temp);
            }
...
<form action="addmeetingjava" method="post" onsubmit="return validateForm()">
    <div class="container">
        <div class="form-section">
            <h1 class="text-center margin-top">Randevu Ekle</h1>
            <div class="form-group">
                ...
                <select id="department" name="department" class="form-control modern-select" required>
                    ...
                    <% for(int i=0; i<10;i++){
                        if(i%2==0){
                    %>
                    <option value="<%=i+1%>"><%=doctors.get(i)[0]%></option>
                    <%}}%>
                </select>
            </div>
            ...
            <div class="form-group">
                ...
                <input type="text" id="appointment-date" name="appointment-date" class="form-control" placeholder="Tarihi seçin" required>
            </div>
...
<script>
    ...
    $('#department').on('change', function() {
        ...
        var doctor = {
            '1':[
                {id: '1', name:'<%= doctors.get(0)[1]%>'},
                {id: '2', name:'<%= doctors.get(1)[1]%>'}
            ],
            '2':[
                {id: '3', name:'<%= doctors.get(2)[1]%>'},
                {id: '4', name:'<%= doctors.get(3)[1]%>'}
            ],
            '3':[
                {id: '5', name:'<%= doctors.get(4)[1]%>'},
                {id: '6', name:'<%= doctors.get(5)[1]%>'}
            ],
            '4':[
                {id: '7', name:'<%= doctors.get(6)[1]%>'},
                {id: '8', name:'<%= doctors.get(7)[1]%>'}
            ],
            '5':[
                {id: '9', name:'<%= doctors.get(8)[1]%>'},
                {id: '10', name:'<%= doctors.get(9)[1]%>'}
            ]
        };
        var doctors = {
            '1': [
